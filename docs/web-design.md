...
- 단, 10개 메뉴·theme·입력 상태·결과 binding처럼 이 문서가 명시한 canonical contract는 기존 구현의 다수결이 아니라 **모든 페이지가 이동할 목표 규칙**이다.
...
packages/
  ...
  shell/       10개 메뉴, theme, page header, status, disclosure
...
4. 10개 메뉴, visible copy, typography, spacing, token, chart, table, overflow inventory를 만들어.
