...
- **visible prose는 이 다수결 규칙의 대상이 아니다.** Hero 설명, section 설명, chart 도움말, disclaimer의 위치와 문구는 여러 페이지에서 반복되어도 공통 기본값으로 승격하지 않는다.
- 현재 6개 사이트의 레이아웃·밀도·상호작용은 기준이지만, 화면에 남아 있는 설명 문구는 보호 대상이나 신규 프로젝트의 복사 원본이 아니다. 매 작업에서 다시 필요성을 감사한다.
- 단, 11개 메뉴·theme·입력 상태·결과 binding처럼 이 문서가 명시한 canonical contract는 기존 구현의 다수결이 아니라 **모든 페이지가 이동할 목표 규칙**이다.
- 프로젝트 분석 의미 때문에 필요한 차이는 허용된 variant로 기록한다.
- 단순히 과거 구현이 달랐다는 이유로 새 variant를 만들지 않는다.
- 한 사이트를 통째로 복사하지 않는다. **여섯 사이트의 공통 문법**을 사용한다.

### 0.3 기준 사이트의 결함은 기준이 아니다

여섯 사이트는 디자인 우선순위이지, 현재 구현의 모든 동작이 정답이라는 뜻은 아니다. `2026-07-24` 차트·입력 감사에서 발견한 아래 결함은 신규 구현에 복사하지 않고 기존 페이지에서도 교정 대상으로 취급한다.

- Best Factor·Momentum의 line chart는 SVG `viewBox`와 실제 렌더 비율이 다른데 전체 bounding box를 선형 환산해 포인터 위치와 날짜가 어긋나고 양 끝 구간을 선택할 수 없다.
- Fear & Greed의 scatter chart도 `preserveAspectRatio`로 생긴 여백을 무시해 포인터와 실제 점의 좌표가 어긋난다.
- ETF Tracking은 elapsed-time 축으로 그리면서 hit-test는 관측 배열의 균등 index를 사용해 불규칙 날짜에서 다른 날짜가 선택된다. 기간 종료일을 바꾸면 전역 최신값과 선택 기간의 snapshot이 한 화면에 섞이고, 빈 기간이 최신값으로 조용히 fallback된다.
- SOX quadrant는 점 반경과 focus halo를 위한 경계 여백이 부족해 극단값이 잘리고 점이 겹친다. hover preview와 click pin 상태, React와 static fallback의 선택 동작도 동일하지 않다.
- Best Factor·Momentum의 정적 화면은 실행 backend가 준비되지 않으면 분석 draft만 바뀌고 결과는 바뀌지 않는다. 이 상태를 정상 실행 가능한 분석 UI처럼 보이게 하지 않는다.

공통 규칙은 위 결함을 제거한 계약과 검증 결과에서만 추출한다.
...
- 분석 universe·portfolio·result를 결정하는 계산·normalization·selection·aggregation 함수는 동결한다.
- DOM hit-test, hover/pin, 차트 관찰일 snap처럼 결과 identity를 바꾸지 않는 presentation selection은 underlying row·series·point·value를 보존하는 범위에서 수정할 수 있다.
...
결과·표시·운영 상태를 바꾸는 control registry와 별도로 nav·theme·disclosure·anchor 같은 모든 visible interactive element의 안정적 id·role·accessible name inventory를 유지한다. `operation`은 manifest 이름만이 아니라 실제 workflow/API의 독립 실행 mode와 일치할 때만 화면에 노출한다.
...
| 프로젝트 | 현재 입력 경계 | 구현 원칙 |
| --- | --- | --- |
| DRAM | 제품·출처·지표·차트 관찰일 등의 `display` | 저장 관측만 사용한다. 차트 관찰일은 현재 결과 안의 readout을 바꾸는 표시 상태이며 재계산이나 새 result identity로 표현하지 않는다. |
| ETF Tracking | ETF 선택은 `result_selector`; 차트 관찰일·기간 시작·기간 preset은 `display`; 기간 종료일은 전체 화면이 같은 과거 snapshot에 결합될 때만 `result_selector`; 인증 작업은 `operation` | 저장 이력 탐색과 수집 작업을 분리한다. `result_selector`를 바꾸면 Hero·KPI·차트·표·신호·출처가 모두 같은 effective snapshot을 사용한다. |
| SOX | 종목·검색·정렬 등의 `display`, 저장 snapshot 날짜 1개 `result_selector`, 현재 하나의 결합된 refresh→publish owner workflow | 저장 기준일·종목을 선택한다. manifest에서 refresh·publish를 독립 operation처럼 나누려면 실제 workflow도 두 mode를 독립 실행할 수 있어야 한다. |
...
- `draft_config != applied_config`가 되는 즉시 `미적용 변경` 상태와 현재 결과의 적용 기준을 입력 panel 가까이에 표시한다. 실행 성공 전까지 공식 결과는 기존 `bound_result`에 결합한다.
- 제출 전에 정적 preset·remote API·local API 중 실제 사용 가능한 실행 경로를 판정한다. 사용할 수 없으면 CTA를 비활성화하고 `실행 연결 필요`처럼 이유와 필요한 조치를 정확히 표시한다.
- 실제 실행 경로 없이 command만 만드는 화면은 command export 기능으로만 명명하고, 결과가 바뀐다고 암시하지 않는다.
- `display` 변경 전후에는 config hash, run id, Python command와 저장 결과가 변하지 않아야 한다.
- `result_selector`는 새 계산을 만들지 않고 존재하는 result identity만 선택한다. 선택 후 Hero·KPI·차트·표·신호·출처 등 모든 result-bound surface는 같은 result key·기준일·snapshot identity를 사용한다.
- `result_selector`의 범위가 비었거나 역전됐거나 요청 결과가 없을 때 범위 밖 최신값으로 조용히 fallback하지 않는다. 프로젝트가 선언한 validation·clamp·snap·empty 정책으로 닫고 requested/effective selector를 구분한다.
...
- 한 canonical parameter에는 한 개의 쓰기 가능한 control만 둔다. 거래일 수처럼 다른 입력에서 파생되는 값은 read-only로 표시하거나 `input` event부터 동기화하고 submit-before-blur도 검증한다.

기간은 역할을 생략한 `기간`으로만 쓰지 않는다.

- `차트 표시 기간`, `차트 관찰일`: `display`
- `저장 결과 기준일`, `저장 preset`: `result_selector`
- `분석 평가 기간`, `학습 기간`, `cutoff`: `analysis`

시작일을 바꿔도 과거 전용 학습 때문에 종료일 신호가 합법적으로 같을 수 있다. 이 경우도 silent no-op로 처리하지 말고 training/evaluation/cutoff 범위와 no-op 사유를 계약·결과 metadata·결정적 fixture에 명시한다.

정규화 뒤 effective config·유효 표본이 이전과 같아 결과가 같은 합법적 no-op이면 `no_op_reason`, effective range와 유효 관측·세션 수를 제공한다. 단순히 화면이 그대로이거나 config hash만 바뀐 상태를 합법적 no-op로 추정하지 않는다.
...
모든 parameter를 같은 숫자 하나로 검증할 필요는 없지만, 각 parameter family는 책임지는 결과를 가져야 한다. 특히 분석 평가 기간은 command 문자열이나 provider 다운로드 인자만 바꾸는 것으로 통과시키지 않는다. 결정적 fixture에서 effective observation boundary·평가 구간·그 구간에 의존하는 결과 중 하나가 실제로 달라져야 한다. 합법적 data-insensitive no-op는 effective input과 새 identity가 기록되고 사유가 계약에 선언된 경우에만 허용한다.
...
packages/
  ...
  shell/       11개 메뉴, theme, page header, status, disclosure
...
| 8 | Risk Score | `https://sonchanggi.github.io/quant-dashboard/risk-score/` |
| 9 | Port | `https://sonchanggi.github.io/port/` |
| 10 | Valuation | `https://sonchanggi.github.io/valuation/` |
| 11 | Kelly | `https://sonchanggi.github.io/kelly/` |
...
`760px` 이하에서는 brand와 theme control을 첫 행에 유지하고, 11개 링크를 두 번째 행의 **메뉴 자체 horizontal rail**로 제공한다. 링크를 문서 밖으로 넘기거나 일부만 숨기지 않는다.
...
- eyebrow와 한 개의 `h1`을 기본 구성으로 사용하며 **설명 문장의 기본 개수는 0개**다.
- Hero 설명은 동적 결과·오해 방지·비표준 필수 행동·법적 또는 데이터 권리 고지 중 하나를 제목·값·상태로 전달할 수 없을 때만 한 문장 허용한다.
- 화면 아래에 무엇이 있는지 예고하거나 사용 순서를 설명하기 위해 Hero 설명을 추가하지 않는다.
...
4. plot
5. 접근 가능한 정확값 표
6. 비표준 조작에만 필요할 경우 접근성 채널 또는 기본 닫힘 도움말
...
클릭·탭·hover·focus·화살표·`Home`·`End`처럼 웹과 차트에서 예측 가능한 조작은 상시 visible 문장으로 열거하지 않는다. 기능은 그대로 유지하고 native semantics, accessible name, `aria-keyshortcuts`, `sr-only` 안내 또는 기본 닫힘 `차트 도움말`로 제공한다. 시각적 안내가 없으면 실제로 수행할 수 없는 프로젝트 고유 조작만 control 가까이에 짧게 표시할 수 있다.
...
- `preview`와 `pinned` 상태를 같은 변수로 합치지 않는다. hover·focus만으로 영구 선택이 바뀌어서는 안 된다.
...
- 선택 point와 halo를 표시
- 휴장일·누락일 처리는 기존 프로젝트 규칙을 유지하되, 선택 domain이 모든 계열 날짜의 합집합인지 active series의 실제 관측일인지 선언한다.
- 합집합 날짜에서 active series 값이 없으면 `관측 없음`을 표시하거나 가장 가까운 실제 관측점으로 snap한다. 과거 값을 현재 선택일 값처럼 조용히 carry-forward하지 않는다.
- readout 날짜는 marker와 값이 실제로 가리키는 관측일과 같아야 한다.
...
- click·tap은 직전 hover 상태가 아니라 자기 event 좌표로 독립적으로 선택을 확정한다.
...
차트가 그리는 좌표와 포인터가 선택하는 좌표는 **같은 scale·domain·range·plot bounds**를 사용한다.

상호작용 차트는 코드 또는 test manifest에 `ChartInteractionContract`를 둔다. 최소 `viewBox/coordinateSpace`, `plotBounds`, `x/y scale`, `selectionDomain`, `missingObservationPolicy`, `snapPolicy`, `preview/pin state`, `pointer/touch/keyboard modes`, `scroll/resize policy`를 선언한다.

- SVG의 client 좌표는 `DOMPoint` 또는 `createSVGPoint()`와 `getScreenCTM().inverse()`로 SVG user space에 변환한다.
- `getBoundingClientRect()` 전체 폭을 `viewBox` 폭으로 단순 비례 환산하는 방식은 `preserveAspectRatio="none"`을 의도적으로 사용하고 실제 렌더 비율이 검증된 경우가 아니면 금지한다.
- CSS의 고정 height와 `width: 100%`가 `viewBox` 비율을 깨뜨려 letterbox를 만들지 않게 한다. `height: auto` 또는 일치하는 `aspect-ratio`를 우선하되 CTM 기반 hit-test를 유지한다.
- time scale로 그린 날짜는 같은 time scale로 hit-test한다. 불규칙 관측일을 배열 index의 균등 간격으로 근사하지 않는다.
- plot margin·끝 label gutter·horizontal scroll·zoom·responsive resize를 좌표 변환에 포함한다. `ResizeObserver` 또는 동등한 재계산으로 실제 크기 변경을 반영한다.
- pointer hit layer는 실제 plot bounds를 덮고, 보이는 plot의 첫 점·중간 점·마지막 점을 모두 선택할 수 있어야 한다. 장식용 letterbox나 gutter를 data domain으로 계산하지 않는다.
- `pointermove`, 직접 click, tap, date input, `ArrowLeft/Right`, `Home/End`는 같은 selection resolver를 사용한다.

실제 브라우저 E2E에서 desktop wide·narrow, component breakpoint 전후 `±1px`, `390px` horizontal scroll, `scrollLeft != 0`, zoom `100%·125%·150%`의 plot `0·25·50·75·100%`를 검사한다. 첫·중간·마지막·single-point·불규칙 날짜 fixture를 포함하고, 선택 오차는 최대 반 관측 간격 또는 `4px` 중 더 엄격한 기준을 넘지 않는다. helper 단위 테스트나 listener 문자열 존재만으로 통과시키지 않는다.

### 10.8 경계·충돌·renderer parity

- scatter·quadrant는 point radius와 focus halo만큼 내부 plot padding을 예약한다. 극단값 `0/1`, 긴 label, `320px·390px`에서 점·label이 잘리거나 겹치지 않아야 한다.
- 충돌 처리는 재현 가능한 deterministic rule을 사용하고, 원자료 좌표를 임의 jitter로 왜곡하지 않는다. 정확값은 plot 밖 readout으로 보존한다.
- bar의 값이 `0`이면 길이도 `0`이어야 한다. 최소 click target과 최소 시각 길이를 데이터 magnitude에 섞지 않는다.
- 음수 bar는 zero baseline과 방향을 표현한다. 그렇지 않으면 component를 nonnegative-only로 선언한다.
- React와 static fallback처럼 renderer가 둘 이상이면 하나를 canonical build output으로 정하거나, 같은 fixture에서 selection·readout·차트·표·keyboard 동작의 parity를 검증한다.

## 11. UX writing

- 결론·기준일·핵심 수치를 먼저 쓴다. `다음 행동`은 입력 누락, 오류 복구, 실행 가능한 CTA처럼 실제 행동이 필요할 때만 쓴다.
- 같은 내용을 title, badge, helper, footer에서 반복하지 않는다.
- 제목·값·control로 의미가 분명하면 helper를 쓰지 않는다.
- Hero·section·chart의 supporting copy는 모두 기본 0개다.
- 정보 구조 지시를 사용자용 문장으로 번역하지 않는다. 화면이 이미 구현한 순서와 구성을 다시 설명하지 않는다.
- `먼저 보여줍니다`, `차례로 확인합니다`, `이 페이지에서는`, `아래에서 확인합니다` 같은 화면 순서·내용 예고 문구를 사용하지 않는다.
- 제목, 카드, 차트, 표에 이미 나타난 명사를 나열하고 `보여줍니다`, `확인합니다`, `비교합니다`로 끝내는 문장을 사용하지 않는다.
- 클릭·탭·포인터·hover·화살표 키·`Home`·`End` 같은 표준 조작 목록을 상시 노출하지 않는다.
...
### 11.1 visible supporting copy 유지 계약

Hero·section·chart의 설명을 화면에 남기려면 프로젝트가 소유하는 inventory에 다음을 모두 기록한다.

| field | 계약 |
| --- | --- |
| `selector` 또는 안정적인 copy id | 렌더링된 문장을 한 곳으로 식별 |
| `text` 또는 정확한 matcher | 실제 visible 문구 |
| `intent` | `dynamic-result`, `interpretation-guardrail`, `nonstandard-action`, `required-disclosure` 중 하나 |
| `reason` | 제목·값·상태·control만으로 전달할 수 없는 새 정보와 삭제 시 생기는 실제 오해 |
| `max_occurrences` | 원칙적으로 `1` |

유지 사유는 “이해를 돕기 위해”, “UX를 위해”, “공통 디자인이므로”처럼 일반적이어서는 안 된다. 다음 네 intent 외에는 visible supporting copy를 허용하지 않는다.

- `dynamic-result`: 현재 입력·선택·데이터에 따라 달라지는 결론 또는 상태
- `interpretation-guardrail`: 분석 결과를 반대로 해석할 가능성을 막는 프로젝트 고유 조건
- `nonstandard-action`: control label과 일반 웹 관습만으로 수행 방법을 알 수 없는 필수 조작
- `required-disclosure`: 법적·계약상 또는 데이터 권리상 공개 화면에 한 번 표시해야 하는 고지

단순한 페이지 투어, 아래 section의 내용 예고, 제목의 산문형 반복, 표준 조작법, frontend·backend 구현 설명은 intent가 아니다. 프로젝트별 유지 문구와 사유는 각 프로젝트의 config가 소유하며 공통 registry나 template에 넣지 않는다.

### 11.2 접근성 문구와 상시 visible 문구를 분리한다

- accessible name, `aria-describedby`, `aria-keyshortcuts`, `sr-only` 문구는 기능 접근성을 위해 보존한다.
- 접근성 문구를 지웠다는 이유로 copy 감소를 달성했다고 보고하지 않는다.
- 반대로 접근성에 필요하다는 이유로 키보드·포인터 조작 설명을 본문에 계속 노출하지 않는다.
- `데이터 · 출처 · 운영 상세` 안의 운영 문구는 main canvas copy와 별도 채널로 검사하되, 중복·불필요한 구현 설명은 그 안에서도 허용하지 않는다.
- interaction test는 특정 한국어 문장의 존재가 아니라 실제 pointer·click·tap·keyboard 후 선택 상태, 정확값, focus와 ARIA를 검증한다.

### 11.3 재사용 copy gate

공통 dependency-free CLI는 `scripts/copy-audit.mjs`, 프로젝트별 schema 예시는
`docs/copy-audit.config.example.json`이다.

```bash
node scripts/copy-audit.mjs --config path/to/copy-audit.config.json
```

- `rendered-html` target을 최종 판정에 사용한다. React·TypeScript와 정적 HTML 모두 기본·선택·loading·empty·degraded·error 상태를 브라우저에서 렌더링한 fixture로 검사한다.
- `source` target은 HTML·JS·JSX·TS·TSX의 동적 문구 후보를 빠르게 찾는 보조 검사다.
- `data-copy-role="hero-support|section-support|chart-instructions"`로 기본 0 gate를 적용한다.
- `data-copy-channel="sr-only|operations|runtime-status|validation"`로 접근성·운영 상세·실행 상태·입력 오류 채널을 visible main canvas supporting copy와 분리한다.
- source 상수는 인접한 `// @copy-channel runtime-status|validation` marker 또는 프로젝트 config의 좁은 `sourceChannelRules`로 분류한다. `status|error|message`처럼 넓은 제외 정규식은 금지한다.
- runtime·validation 문구는 copy gate에서 제외하되 실제 상태 전환, `aria-live`, 입력 근처 오류와 복구 행동을 별도 interaction test로 검증한다.
- 유지 문구는 프로젝트 config의 `retainedVisibleCopy`가 `copyId` 또는 정확한 `text`, intent, reason, 최대 노출 횟수를 소유한다.
- 공통 CLI에는 특정 프로젝트 문장이나 프로젝트별 allowlist를 하드코딩하지 않는다.
...
4. 공개 화면과 source에서 다음 inventory를 만든다.
   ...
   - 모든 visible copy, 중복, `data-copy-role`, 유지할 supporting copy의 intent·구체적 reason
   - computed font-size·weight·line-height
   - spacing·radius·token
   - chart type·series·axis·legend·readout·selection domain
   - SVG `viewBox`·실제 rendered rect·plot bounds·hit layer·client↔data 좌표 변환
   - 불규칙 날짜·첫/중간/마지막 점·direct tap·scroll·resize에서 실제 선택 결과
   - React/static 등 중복 renderer와 동작 parity
   - table column·order
   - 긴 페이지의 상단·하단 빠른 이동
   - desktop/mobile scrollWidth·scrollHeight
5. 현재 프로젝트의 좋은 구성은 유지하고 이 문서와 다른 drift만 수정한다.
6. Hero·section·chart supporting copy를 우선 0개로 만들고, 유지할 문장만 프로젝트별 allowlist에 사유와 함께 등록한다.
7. 실제 브라우저 또는 대표 상태 fixture의 **렌더링된 DOM**으로 copy audit를 실행한다. source 정적 검사는 보조 수단이며 동적 문구의 최종 증거가 아니다.
8. 문구 자체를 요구하는 interaction test를 실제 상태·값·ARIA 동작 테스트로 교체한다.
9. 각 `analysis` 입력의 실제 A/B artifact 변화, `result_selector`의 전 surface identity 일치, `display`의 결과 불변을 검증한다.
10. 차트는 실제 브라우저 rendered geometry에서 포인터·tap·keyboard·경계·scroll을 검증한다.
...
7. 결과 fixture, 각 `analysis` control family의 결정적 A/B fixture, `result_selector`의 동일 snapshot fixture, `display` control의 불변 fixture를 먼저 고정하고 UI·worker가 같은 계약을 지키는지 검증한다.
8. template의 Hero·section 설명은 비워 두고, 필요한 문장만 프로젝트별 retained-copy inventory에 intent·reason과 함께 추가한다.
9. 대표 상태를 렌더링한 DOM fixture와 프로젝트별 copy-audit config를 만들고 CI에서 검사한다.
10. date/scatter chart는 같은 scale을 쓰는 공용 selection resolver와 CTM 기반 SVG coordinate helper를 사용하고 실제 브라우저 geometry matrix를 통과시킨다.
...
- 분석 평가 기간은 command·download 인자만이 아니라 effective observation boundary 또는 기간 의존 결과의 변화를 결정적 fixture로 증명해야 한다.
- `display` 변경 전후 config hash·run id·Python command·결과 JSON이 같아야 한다.
- `draft_config != applied_config`일 때 미적용 상태가 보이고, backend route가 없으면 실행 CTA와 이유가 정확히 닫혀야 한다.
- 하나의 canonical parameter에 쓰기 가능한 control이 둘 이상 없어야 하며 파생값은 submit-before-blur에서도 동기화되어야 한다.
...
- `result_selector` 변경 후 Hero·KPI·차트·표·신호·출처의 result key·기준일·snapshot identity가 모두 같아야 한다.
- 빈·역전·존재하지 않는 selector 범위가 범위 밖 최신 결과로 조용히 fallback되지 않아야 한다.
...
- 구현·재계산 설명 0건
- Hero·section supporting copy 기본 0건, 예외는 프로젝트별 intent·구체적 reason·최대 노출 횟수가 등록됨
- 화면 순서 안내·하위 section 예고·제목의 산문형 반복 0건
- 표준 pointer·click·tap·hover·keyboard 조작의 상시 visible 설명 0건
- `sr-only`·ARIA·닫힌 도움말의 접근성 정보는 실제 interaction과 함께 보존됨
- rendered-DOM copy audit 통과; source 정적 검사만으로 완료 처리하지 않음
- 특정 안내 문구의 존재가 아니라 interaction 결과를 검증함
- 일반 운영·한계 상세는 닫힌 한 곳
...
- SVG client 좌표는 CTM-aware하게 변환되고 plot renderer와 hit-test가 동일한 scale·domain·range를 사용함
- 실제 브라우저에서 첫·중간·마지막·single-point·불규칙 날짜, direct tap, horizontal scroll, resize, zoom 선택 오차가 허용 범위 안임
- readout 날짜·marker·값이 같은 실제 관측점을 가리키며 양 끝에 선택 불가 구간이 없음
- scatter/quadrant 극단값과 focus halo가 잘리지 않고, bar `0`이 양의 magnitude처럼 보이지 않음
- hover/focus preview가 leave/blur 뒤 pinned 상태로 복귀하고 click/tap/Enter/Space만 pin을 바꿈
- duplicate renderer의 selection·readout·chart·table behavior parity가 같은 fixture에서 검증됨
...
14. Hero·section·chart에 남은 visible supporting copy는 모두 허용 intent와 구체적 유지 사유를 가지며, 화면 순서·표준 조작·구현 설명은 0건이다.
15. 포인터·tap·keyboard가 같은 실제 관측점을 선택하고, 첫 점과 마지막 점을 포함해 선택 불가능한 plot 구간이 없다.
16. 분석 draft와 적용 결과가 분리되고, 기간을 포함한 모든 분석 입력은 실행 가능한 경로와 결정적 A/B 결과 증거를 가진다.
...
```text
[프로젝트명] 웹페이지를 최신 quant-dashboard/docs/web-design.md 기준으로 구현·개선해줘.
Python 분석·JSON·결과 계약은 바꾸지 말고, 모든 차트의 포인터·양 끝 선택과 모든 입력의 display/result_selector/analysis 결합을 실제 브라우저와 A/B 결과로 검증해.
6개 기준 사이트와 통일하되 프로젝트 고유 구조는 보존하고, 로컬 미리보기까지만 진행해. 배포는 내 승인 후에만 해.
```
...
- draft가 적용값과 달라지면 미적용 상태를 표시하고, 실제 backend route가 없으면 실행 CTA를 비활성화해 이유를 알려줘.
...
4. 11개 메뉴, visible copy, typography, spacing, token, chart, table, overflow inventory를 만들어.
...
8. 차트 정확값을 plot 밖에 두고, 해당 기능이 있는 차트에서만 선택 계열 강조·날짜·keyboard·mobile 동작을 검증해. SVG 좌표는 CTM-aware하게 변환하고 renderer와 hit-test가 같은 scale을 쓰는지 실제 브라우저의 첫·중간·마지막·불규칙 날짜·direct tap·scroll로 확인해.
