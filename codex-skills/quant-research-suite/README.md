...
current-user, same-request metadata generated by that selector. Semantic,
plain, quoted, negated, prior-turn, Goal, artifact, and worker context do not
activate it.
...
The public files contain role-specific behavior. Non-trivial work loads the
lean kernel at `shared/references/adaptive-workflow.md`; small tasks skip it.
The kernel:

- grounds work in the actual target and available capabilities;
- asks only for material user-owned decisions;
- uses subagents for independent lanes and teams only for ongoing coordination;
...
- selects only zero-billing data routes and proves the actual consumer surface.
