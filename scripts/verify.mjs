...
assert(contains(files.app, 'validMomentumLiveProvenance') && contains(files.app, 'priceSources') && contains(files.app, 'sourceHealth') && contains(files.app, 'MOMENTUM_LIVE_SNAPSHOT_HASH_FIELDS') && contains(files.app, "'comparisonPrices'"), 'momentum consumer requires live provider provenance arrays and the exact ten-field snapshot hash contract');
