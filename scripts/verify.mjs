...
assert(contains(files.commonDesign, '버전: `1.2.0`') && contains(files.commonDesign, 'DRAM 기준 타이포그래피와 세로 밀도'), 'common design v1.2 makes DRAM the typography and density reference');
assert(contains(files.commonDesign, '`15px`, `line-height: 1.55`') && contains(files.commonDesign, '`800` 이상'), 'common design v1.2 fixes the DRAM-based type scale and weight hierarchy');
...
assert(contains(files.commonDesign, '11개 항목') && contains(files.commonDesign, 'DRAM·Fear & Greed 또는 현재 프로젝트와 Hub만'), 'common design v1.2 requires the complete shared navigation');
assert(contains(files.commonDesign, '기존 청색 중심 색감') && contains(files.commonDesign, '기존 차트·표·결과 구조를 DRAM 형태로 재설계'), 'common design v1.2 preserves project-specific visuals and analytical surfaces');
...
assert(contains(files.commonDesignPrompt, 'Quant Research 공통 디자인 v1.2') && contains(files.commonDesignPrompt, 'DRAM 기준 타이포그래피와 밀도'), 'rollout prompt applies the DRAM visual reference');
...
assert(contains(files.commonDesignPrompt, 'sticky shell은 약 `58px`') && contains(files.commonDesignPrompt, '메뉴 글자는 `12px / 650`'), 'rollout prompt carries the compact DRAM navigation density');
