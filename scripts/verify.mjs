...
assert(contains(files.app, 'MOMENTUM_SUMMARY_CONTRACT') && contains(files.app, 'expectedVersion: 4'), 'momentum adapter requires schemaVersion 4 summary contract');
assert(contains(files.app, 'selectedFactor') && contains(files.app, 'compositeScore'), 'momentum adapter reads selected factor and composite score fields');
assert(contains(files.app, 'selectedWeightingPolicy') && contains(files.app, 'selectedReason'), 'momentum adapter reads selected weighting policy and joint reason fields');
...
assert(contains(files.app, 'validMomentumLiveProvenance') && contains(files.app, 'priceSources') && contains(files.app, 'sourceHealth') && contains(files.app, 'MOMENTUM_LIVE_SNAPSHOT_HASH_FIELDS'), 'momentum consumer requires live provider provenance arrays and the exact nine-field snapshot hash contract');
...
assert(contains(files.app, 'validMomentumFactorGrid') && contains(files.app, 'factorCount: 64') && contains(files.app, 'independentFactorCount: 61') && contains(files.app, 'aliasFactorCount: 3') && contains(files.app, 'totalPairCount: 256'), 'momentum consumer enforces the canonical 64/61/3 and 244/12/256 factor grid');
...
assert(contains(files.app, 'currentResearchTarget') && contains(files.app, 'summaryTarget') && contains(files.app, 'dashboardTarget'), 'momentum consumer checks full canonical currentResearchTarget parity');
...
assert(contains(files.app, 'momentumDashboard') && contains(files.app, 'momentumRowsFromModelWeights'), 'momentum adapter loads schema v4 model holdings');
assert(contains(files.app, 'currentResearchTarget') && contains(files.app, 'weightingPolicyId'), 'momentum dashboard parser consumes the canonical current research target');
