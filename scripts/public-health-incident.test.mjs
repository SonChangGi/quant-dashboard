...
test('age and date drift do not create a new incident identity', () => {
  assert.equal(
    normalizeIncidentMessage('sox data is 13.3 days old on 2026-08-14; expected <= 5 days'),
    'sox data is # days old on #date; expected <= 5 days',
  );
  const first = incidentState({
    findings: [{ project: 'sox', category: 'freshness', severity: 'hard', message: 'sox data is 13.3 days old' }],
  });
  const second = incidentState({
    findings: [{ project: 'sox', category: 'freshness', severity: 'hard', message: 'sox data is 14.3 days old' }],
  }, first);
...
test('new hard category changes the incident fingerprint', () => {
  const first = incidentState({
    findings: [{ project: 'sox', category: 'freshness', severity: 'hard', message: 'stale for 13 days' }],
  });
  const second = incidentState({
    findings: [{ project: 'sox', category: 'contract', severity: 'hard', message: 'schema mismatch' }],
  }, first);
...
test('transient findings do not enter the hard incident identity', () => {
  const report = {
    findings: [
      ...
      { project: 'sox', category: 'freshness', severity: 'hard', message: 'stale' },
    ],
  };
  assert.deepEqual(incidentIdentity(report), [
    { project: 'sox', category: 'freshness', message: 'stale', affectedProjects: [] },
  ]);
});

test('threshold and affected-project changes create a new incident identity', () => {
  const thresholdFive = incidentState({
    findings: [{ project: 'sox', category: 'freshness', severity: 'hard', message: 'sox data is 14 days old; expected <= 5 days' }],
  });
  const thresholdTen = incidentState({
    findings: [{ project: 'sox', category: 'freshness', severity: 'hard', message: 'sox data is 15 days old; expected <= 10 days' }],
  }, thresholdFive);
  ...
  const twoProjects = incidentState({
    findings: [{
      ...
      severity: 'hard',
      message: '2/7 public projects are unobservable; hard threshold is 2',
      affectedProjects: ['sox', 'etf'],
    }],
  });
  const threeProjects = incidentState({
    findings: [{
      ...
      category: 'observability',
      severity: 'hard',
      message: '3/7 public projects are unobservable; hard threshold is 2',
      affectedProjects: ['sox', 'etf', 'fearngreed'],
    }],
  }, twoProjects);
...
test('unknown previous-state contracts fail safe as a new incident', () => {
  const current = incidentState({
    findings: [{ project: 'sox', category: 'freshness', severity: 'hard', message: 'stale' }],
