...
const degradedOperationalFindings = operationalFindingsFor('etf', {
  state: 'degraded',
  reasonCodes: ['provider_rights_unconfirmed'],
  meta: {
    statusState: 'degraded',
    degradedReasons: ['provider_html_challenge'],
  },
});
assert(
  degradedOperationalFindings.some((finding) => /upstream state is degraded/.test(finding.message))
    && degradedOperationalFindings.some((finding) => /provider_html_challenge/.test(finding.message))
    && degradedOperationalFindings.some((finding) => /provider_rights_unconfirmed/.test(finding.message)),
  'public health reports degraded upstream state and reason codes',
);
assert(
  transportEscalation(['etf'], 7) === null,
  'one transient transport outage remains a soft observation warning',
);
const broadTransportFailure = transportEscalation(['etf', 'port'], 7);
assert(
