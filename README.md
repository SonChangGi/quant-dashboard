...
- Momentum과 Best Factor는 팩터명, 표시 5종목 비중, 종합 점수, 종목별 모델 비중을 표시합니다. 비중 막대는 두 패널이 같은 축(최소 0–40%)을 사용합니다. Momentum 신호 순위와 Best 비중 순위를 유지하며, 표시 종목 비중은 해당 행의 실제 비중을 합산합니다. 원본 팩터 ID·발행 시각·정확한 표는 `팩터 · 데이터 상세`에서 확인합니다.
